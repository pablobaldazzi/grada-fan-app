...
describe("SquadPlayerSchema", () => {
  it("parses a squad player", () => {
    ...
    expect(SquadPlayerSchema.parse(data)).toEqual(data);
  });
});

describe("ClubSquadSchema", () => {
  it("parses the fan-app squad payload", () => {
    const data = {
      name: "4-3-3",
      coach: "Erwin Duran",
      startingEleven: [
    ...
    const parsed = ClubSquadSchema.parse(data);
    expect(parsed.coach).toBe("Erwin Duran");
    expect(parsed.startingEleven[0].position).toBe("GK");
  });
});
