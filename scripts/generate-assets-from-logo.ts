 ...
 * Adds a solid background color for logos with transparency.
 *
...
async function main() {
  ...
  async function createWithBackground(size: number, padding = 0.15) {
  ...
  // Icon 1024x1024
  await (await createWithBackground(1024))
    .png()
  ...
  // Android foreground 432x432 (logo on transparent for adaptive - or with bg)
  await (await createWithBackground(432))
    .png()
  ...
  const mono = await createWithBackground(432);
  await (await mono)
    .grayscale()
    .png()
  ...
  // Favicon 48x48
  await (await createWithBackground(48, 0.1))
    .png()
