...
import type { SquadPlayer } from "@/lib/schemas";
  ...
  return (
    <View style={s.playerContainer}>
      <View
        style={[
          s.playerCircle,
          { backgroundColor: posColor, borderColor: "#fff" },
        ]}
      >
...
export default function PlantelScreen() {
  ...
  const { data: squad, isLoading } = useQuery({
    queryKey: ["club-squad", club?.slug],
    queryFn: () => fetchClubSquad(club?.slug ?? ""),
    enabled: !!club?.slug,
  });
  const formationRows = squad
    ? getFormationRows(squad.name, squad.startingEleven.length)
    : [];
        ...
        {!isLoading && !squad ? (
          ...
          <>
            ...
            >
              ...
              />
              <View>
                <Text style={[s.coachLabel, { color: colors.textTertiary }]}>
                  DIRECTOR TÉCNICO
                </Text>
                <Text style={[s.coachName, { color: colors.text }]}>
                  {squad.coach}
                </Text>
              </View>
              <View
              ...
              >
                <Text style={[s.formationText, { color: colors.primary }]}>
                  {squad.name}
                </Text>
              </View>
            </View>
                ...
                {formationRows.map((row, rowIdx) => (
                  ...
                  >
                    {row.players.map((pIdx) => {
                      const player = squad.startingEleven[pIdx];
                      if (!player) return null;
                      return <PlayerDot key={player.id} player={player} />;
                    })}
                  </View>
                ))}
              </View>
              ...
              {(
                ...
                <View key={item.key} style={s.legendItem}>
                  <View
                    style={[
                      s.legendDot,
                      { backgroundColor: POSITION_COLORS[item.key] },
                    ]}
                  />
                  <Text
                    style={[s.legendLabel, { color: colors.textSecondary }]}
                  >
                    {item.label}
                  </Text>
                </View>
                ...
                >
                  <View
                    style={[
                      ...
                      {
                        backgroundColor:
                          POSITION_COLORS[player.position] + "25",
                      },
                    ]}
                  >
                    <Text
                  ...
                  <View style={s.subInfo}>
                    <Text
                      style={[s.subName, { color: colors.text }]}
                      numberOfLines={1}
                    >
                      {player.name}
                    </Text>
                    <Text
                      style={[s.subRole, { color: colors.textTertiary }]}
                      numberOfLines={1}
                    >
                      {player.role}
                    </Text>
